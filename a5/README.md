...
Deadline is oct 8, few things to keep in mind is the files present in program_file.txt will only complied for simulation purpose if you are adding new files then you must add the files into the program_file.txt for example if you are keeping an alu.v module and you are not compiling it the testbench will not reconginze the error , to not get the error you should add the alu.v into the file program.txt
